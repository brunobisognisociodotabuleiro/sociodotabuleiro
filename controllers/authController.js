...
module.exports = {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found') {
        try {
          ...
          res.status(500).json({ status: false, error: "Erro na Criação de usuário" })
        }
      }
    }
  },
  loginUser: async (req, res) => {
    try {
      ...
      !user && res.status(200).json("Credenciais Invalidas")
      const decryptedpassword = CryptoJS.AES.decrypt(user.password, process.env.SECRET)
      const decrypt = decryptedpassword.toString(CryptoJS.enc.Utf8); 

      decrypted !== req.body.password && res.status(401).json("Senha invalida")
